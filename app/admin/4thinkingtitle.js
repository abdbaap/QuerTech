...
export async function DecidingTitleFolder(points) {
    ...
    const prompt=`Act as a Strategic Copywriter and Digital Marketer to generate a high-CTR, click-worthy link title for given summary. Craft a single, punchy title  without any special characters  that utilizes compelling "Power Words" to immediately communicate the primary value proposition or the specific solution found in the content.And text should be like in simple language which is generally searched and associated with the content The title must balance human curiosity with search relevance, ensuring it sounds authoritative rather than like generic clickbait. Avoid all filler and introductory phrasing to provide a direct, high-impact headline that maximizes click-through rates for given summary. and return me object as .json file format the object format should be exactly like this  {
 ...
 const jsonObject=JSON.parse(TitleForNewBlog)
    ...
    return { success: false, error: "AI output was not valid JSON.", message:`there was an error generating the TitleFolder, ${error}` };
  }
}