...
export default function Aiblogssection() {
  return (
      ...
      {aiblogs.map((item,i) => { 
        return <Link href={`${item.link}/index.html`} key={i} className={`flex flex-col rounded-xl shrink-0 gap-2 z-0 hover:z-10 snap-center   p-8  shadow-2xs hover:shadow-xl justify-center text-center items-center xl:w-80 w-75 h-110 hover:h-116 text-white ${item.color}`}>
