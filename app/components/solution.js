...
export default function SolutionsBlog() {
  return (
    <div id='solution' className="flex w-full bg-gray-600 text-center  flex-col justify-center  gap-8 py-8">
        ...
        <div className="flex noscrollbar overflow-x-auto w-full max-h-100vh snap-center snap-x snap-mandatory px-10 xl:px-auto items-center gap-16 ">
{SolutionBlogs.map((item,index) => { return(
    <Link target='_blank' href={`https://quertech-articles.vercel.app/${item.link}/index.html`} key={index} className="flex hover:tracking-wider transition-all duration-500 flex-col bg-gray-950 text-white justify-center xl:w-80 w-75 shrink-0 rounded-xl items-center p-8 gap-2">
