...
{MotivationBlogs.map((item,index) => { 
    return (<Link target='_blank' href={`/${item.link}/index.html`} key={index} className="motivationcard flex noscrollbar  text-black gap-2 overflow-x-hidden overflow-y-scroll  justify-evenly bg-gray-400 rounded-xl shadow-sm hover:shadow-2xl p-1 w-80 h-110 snap-center items-center text-center shrink-0">
