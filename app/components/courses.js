  ...
  return (
    <div id='courses' className="flex  flex-col w-full   gap-8  text-center h-[150%] py-32 overflow-x-hidden overflow-y-scroll shrink-0 ">
        ...
        <div className="flex px-2 flex-wrap w-full justify-evenly text-black items-center gap-16">
            {courses.map((item,I) => { 
             return   <Link style={{ backgroundColor: 'black' }} target='_blank' href={`https://articlesstack.pages.dev/${item.link}/index.html`} key={I} className={` ${item.color} flex flex-row justify-evenly m-0 text-white   items-center  rounded-xl w-80   shadow-xl hover:shadow-2xl gap-2 overflow-x-hidden overflow-y-auto py-2 shrink-0 p-2`}>
                    <div className="flex flex-col justify-center   text-center  items-center gap-2">
                        <Image loading="lazy" src="/courses.webp" width={40} height={40} alt={item.tags}></Image>
