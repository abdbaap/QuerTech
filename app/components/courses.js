  ...
  return (
    <div id='courses' className="flex  flex-col w-full   gap-8  text-center h-[150%] py-32 overflow-x-hidden overflow-y-scroll shrink-0 ">
        ...
        <div className="flex px-2 flex-wrap w-full justify-evenly text-black items-center gap-16">
            {courses.map((item,I) => { 
             return   <Link style={{ backgroundColor: 'black' }} target='_blank' href={`https://quertech-articles.vercel.app/${item.link}/index.html`} key={I} className={` ${item.color} flex flex-row justify-evenly m-0 text-white   items-center  rounded-xl w-80   shadow-xl hover:shadow-2xl gap-2 overflow-x-hidden overflow-y-auto py-2 shrink-0 p-2`}>
