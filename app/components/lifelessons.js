    ...
    <div id='Life Lessons' className="flex flex-col justify-center w-full p-8 h-full  gap-8 items-center">
        ...
        <div className="flex flex-wrap justify-center items-center w-full LifeLessons h-full relative ">
          ...
          {LifeLessons.map((e,id) => { 
            return     <Link target='_blank' href={`https://quertech-articles.vercel.app/${e.link}/index.html`} key={id} className={`aiCard hover:shadow-2xl hover:grow-[0.01] shadow hover:zoom-in-95 text-white relative p-8 flex justify-center items-center  rounded-xl w-[320px] flex-col ${e.color} ${e.rotation}`}>
